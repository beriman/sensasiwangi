...
import PollCreator from "@/components/forum/polls/PollCreator";
import PollViewer from "@/components/forum/polls/PollViewer";
import ThreadSubscription from "@/components/forum/ThreadSubscription";
import {
  MessageSquare,
  ThumbsUp,
  Award,
...
export default function ForumThreadDetail({ threadId }: ForumThreadDetailProps) {
  ...
  useEffect(() => {
    const fetchThreadDetails = async () => {
      try {
        ...
        // Set poll ID if thread has a poll
        if (threadData.has_poll && threadData.poll_id) {
          setPollId(threadData.poll_id);
        }
  ...
  const handlePollCreated = async (newPollId: string) => {
    try {
      // Update thread to link it with the poll
      ...
      console.error("Error updating thread with poll:", error);
      toast({
        title: "Error",
        description: "Failed to link poll to thread",
        variant: "destructive",
      });
    }
  };
            ...
            <div className="flex-1">
              ...
              {pollId && (
                <div className="mt-6 mb-6">
                  <PollViewer pollId={pollId} threadId={threadId} />
                </div>
              )}
              ...
              <div className="flex items-center gap-4 mt-6">
                ...
                {user && thread.user_id === user.id && !thread.has_poll && !showPollCreator && (
                  <Button
                    variant="outline"
                    size="sm"
