...
export default function ProductCard({
  ...
  useEffect(() => {
    const checkWishlistStatus = async () => {
      ...
      try {
        const wishlisted = await isInWishlist(product.id, user);
  ...
  const handleWishlistToggle = async (e: React.MouseEvent) => {
    ...
    setIsLoading(true);
    try {
      const newWishlistState = await toggleWishlist(product.id, user);
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className={`p-1.5 rounded-full bg-white shadow-sm border border-gray-100 transition-all duration-200 hover:bg-gray-50 focus:outline-none focus:ring-2 
focus:ring-purple-500 focus:ring-opacity-50 ${isWishlisted ? "opacity-100" : "opacity-0 group-hover:opacity-100"} ${isLoading ? "animate-pulse" : ""}`}
